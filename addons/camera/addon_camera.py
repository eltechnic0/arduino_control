...
class AppAddon(object):
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def characterize(self, device, spotsize=15, steps=10, radii, pins=(3,9,10,11)):
        """
        ...
        Coordinates that could not be recognized have a value of None. Those that
        were not recognized with the requested `spotsize` have a value `isok` False
        ...
        for radius in radii:
            for k in range(steps):
                ...
                res = cherrypy.engine.publish('serial-write', ['vset', pins, values, settling])
                if not res['success']:
                ...
                if not coords:
                    ...
                    if not coords:
                        ...
                        data.append((radius,valx,valy,coords[0],coords[1],False))
                    continue
